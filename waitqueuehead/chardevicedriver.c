...
static ssize_t chardevicedriver_read(struct file *filp, char __user *buf,
	...
	wait_event_interruptible(wqh, 0!=led);//当条件0!=led为真时,立即返回,否则进程进入TASK_INTERRUPTIBLE类型的睡眠状态
	led = 0;
	printk("Finished to read data from device\n");
...
static ssize_t chardevicedriver_write(struct file *filp, const char __user *buf,size_t count, loff_t *f_pos){
	...
	led = 1;
	wake_up_interruptible(&wqh);//唤醒由queue指向的等待队列头链表中所有等待队列应用的进程
	printk("Finished to write data to device\n");
	return count;
}
...
static int __init chardevicedriver_init(void){
	...
	init_waitqueue_head(&wqh);//定义和初始化队列头
