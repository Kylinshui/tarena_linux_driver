...
static ssize_t chardevicedriver_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos){
	if(async_queue){
		...
		//发送信号SIGIO信号给fasync_struct 结构体所描述的pid,触发应用程序
		//的SIGIO信号处理函数
...
static int chardevicedriver_fasync(int fd, struct file *filp, int mode){
	printk("---%s----\n", __func__);
	//初始化fasync_struct 结构体
	return fasync_helper(fd, filp, mode, &async_queue);
}

static int chardevicedriver_release(struct inode *inode, struct file *filp){
	
	chardevicedriver_fasync(-1, filp, 0);
...
static struct file_operations chardevicedriver_fops = {
	...
	.fasync = chardevicedriver_fasync,
