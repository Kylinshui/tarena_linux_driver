...
char cdd_delayed_work_name[50];
struct delayed_work cdd_delayed_work;
...
static ssize_t chardevicedriver_write(struct file *filp, const char __user *buf,size_t count, loff_t *f_pos){
	//1*HZ后调度函数
	schedule_delayed_work(&cdd_delayed_work, 1*HZ);
	return count;
}
...
static irqreturn_t chardevicedriver_isr(int irq, void *dev_id){
	...
	//schedule_delayed_work(&delayed_work,unsigned long delay)
	return IRQ_HANDLED;
}
...
static int __init chardevicedriver_init(void){
	...
	//初始化延迟工作队列并指定工作函数
	strcpy(cdd_delayed_work_name, "cdd_delayed_work_name");
	INIT_DELAYED_WORK(&cdd_delayed_work, chardevicedriver_cdd_delayed_work_func);
...
static void __exit chardevicedriver_exit(void){
	...
	//刷新工作
	flush_delayed_work(&cdd_delayed_work);
