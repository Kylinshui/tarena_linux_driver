...
struct workqueue_struct *cdd_workqueue;
...
static ssize_t chardevicedriver_write(struct file *filp, const char __user *buf,size_t count, loff_t *f_pos){
	//调度工作,类似于schedule_work,将指定工作work提交给指定工作队列wq
	//driver 调用queue_work向wq中加入工作节点,　work会依次加在cdd_workqueue->worklist所指向的链表中
	queue_work(cdd_workqueue,&cdd_work);
	//queu_delayed_work(&cdd_workqueue, 3*HZ);
	return count;
}
...
static irqreturn_t chardevicedriver_isr(int irq, void *dev_id){
	...
	//schedule_work(&cdd_work);//中断底半部调度工作,这里只是模拟放在write里面
	//queue_work(struct workqueue_struct *wq, struct work_struct *work);
	return IRQ_HANDLED;
}

static void chardevicedriver_cdd_work_func(struct work_struct *work){
	printk("%s  enter...\n",__func__);
...
static int __init chardevicedriver_init(void){
	...
	cdd_workqueue = create_workqueue("add_workqueue");
	if(IS_ERR(cdd_workqueue)){
		printk("Failure to create work queue:cdd_workqueue\n");
		return -1;
	}
	INIT_WORK(&cdd_work, chardevicedriver_cdd_work_func);
...
static void __exit chardevicedriver_exit(void){
	...
	//刷新工作
	flush_workqueue(cdd_workqueue);
	//销毁工作队列
	destroy_workqueue(cdd_workqueue);
